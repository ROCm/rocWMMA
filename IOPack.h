...
struct Pack<float16_t>
{
    ...
    __device__ static inline auto exec(typename Traits::InputT const& input) ->
...
struct Pack<float32_t>
{
    ...
    __device__ static inline auto exec(IncomingT&& input)
        -> decltype(std::forward<IncomingT>(input))
    {
...
struct PackRegs
{
    struct Traits : public Pack<DataT>::Traits
    {
        ...
        static_assert(RegisterCount % Pack<DataT>::Traits::PackRatio == 0,
    ...
    __device__ static inline
        typename std::enable_if<std::is_same<IncomingT, typename Traits::InputT>::value
                                    && (Traits::PackRatio == 1),
                                typename Traits::OutputT>::type
        exec(IncomingT&& input)
    {
        static_assert(std::is_same<IncomingT, typename Traits::OutputT>::value,
        ...
        exec(IncomingT&& input)
    {
        ...
        static_assert(decltype(it)::Range == Traits::PackedRegisterCount,
