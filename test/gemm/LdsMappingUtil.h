                      ...
                      BlocksY>
{
    ...
    */
    ...
    using GlobalReadFragA = wmma::fragment<matrix_a, BlockM, BlockN, BlockK, DataT, LayoutA>;
    using GlobalReadFragB = wmma::fragment<matrix_b, BlockM, BlockN, BlockK, DataT, LayoutB>;

    // Local Write
    // Vertical register file fulfilled by matrix_b with BlockN = 64
    using LocalWriteFragA
        = wmma::fragment<matrix_b, 1, registerFileWidth, GlobalReadFragA::size(), DataT, LayoutLds>;
    using LocalWriteFragB
        = wmma::fragment<matrix_b, 1, registerFileWidth, GlobalReadFragB::size(), DataT, LayoutLds>;
...
public:
    ...
    __device__ static inline uint32_t baseOffsetB()
    {
        ...
        return LdsOffsets::dataOffset(ld(), matrixCoord);
    }
    ...
    __device__ static inline void prefetchGlobalA(DataT* baseLds, DataT const* baseA, uint32_t lda)
    {
        ...
        for(int32_t i = 0; i < BlocksX; ++i)
        {
            ...
            // Issue local store
            wmma::store_matrix_coop_sync(baseLds + baseOffsetA() + waveOffsetA() + blockOffsetA(i),
    ...
    __device__ static inline void prefetchGlobalB(DataT* baseLds, DataT const* baseB, uint32_t ldb)
    {
        // Tricky part: because we may be changing layouts and vector widths,
        // we need to ensure that splitCounts are the same on both sides of
        // global fetch and local writes - Otherwise the waves don't have the
        ...
        for(int32_t i = 0; i < BlocksY; ++i)
        {
            ...
            // Issue local store
            wmma::store_matrix_coop_sync(baseLds + baseOffsetB() + waveOffsetB() + blockOffsetB(i),
    ...
    {
        wmma::load_matrix_sync(reinterpret_cast<LocalReadFragA&>(fragA),
    ...
    {
        wmma::load_matrix_sync(reinterpret_cast<LocalReadFragB&>(fragB),
                      ...
                      BlocksY>
{
    ...
    */
    ...
    using GlobalReadFragA
        = wmma::fragment<matrix_a, BlockM * BlocksX, BlockN, BlockK, DataT, LayoutA>;
    using GlobalReadFragB
        = wmma::fragment<matrix_b, BlockM, BlockN * BlocksY, BlockK, DataT, LayoutB>;

    // Local Write
    // A matrix already aligned
    // B matrix is transposed
    using LocalWriteFragA
        = wmma::fragment<matrix_a, BlockM * BlocksX, BlockN, BlockK, DataT, LayoutLds>;
    using LocalWriteFragB = wmma::fragment<matrix_a,
                                           BlockN * BlocksY,
                                           BlockM,
                                           BlockK,
                                           DataT,
    ...
    using LocalReadFragA = wmma::fragment<matrix_a, BlockM, BlockN, BlockK, DataT, LayoutLds>;
    using LocalReadFragB = wmma::fragment<matrix_a, BlockN, BlockM, BlockK, DataT, LayoutLds>;
    ...
    __device__ static inline uint32_t baseOffsetB()
    {
        ...
        return MappingUtil::dataOffset(ld(), matrixCoord);
    }
    ...
    __device__ static inline void prefetchGlobalA(DataT* baseLds, DataT const* baseA, uint32_t lda)
    {
        ...
        // Issue global read
        GlobalReadFragA fetchA;
        wmma::load_matrix_coop_sync(fetchA, baseA, lda, waveIndex, waveCount, splitCount);

        // Issue local store
        wmma::store_matrix_coop_sync(baseLds + baseOffsetA() + waveOffsetA(),
                                     reinterpret_cast<LocalWriteFragA&>(fetchA),
                                     ld(),
                                     waveIndex,
                                     waveCount,
    ...
    __device__ static inline void prefetchGlobalB(DataT* baseLds, DataT const* baseB, uint32_t ldb)
    {
        ...
        // Issue global read
        GlobalReadFragB fetchB;
        wmma::load_matrix_coop_sync(fetchB, baseB, ldb, waveIndex, waveCount, splitCount);

        // Issue local store
        wmma::store_matrix_coop_sync(baseLds + baseOffsetB() + waveOffsetB(),
                                     reinterpret_cast<LocalWriteFragB&>(fetchB),
                                     ld(),
                                     waveIndex,
                                     waveCount,
                      ...
                      BlocksY>
{
    ...
    */
    ...
    using LocalWriteFragA
        = wmma::fragment<matrix_b, BlockN, BlockM * BlocksX, BlockK, DataT, LayoutLds>;
    using LocalWriteFragB
        = wmma::fragment<matrix_b, BlockM, BlockN * BlocksY, BlockK, DataT, LayoutLds>;

    // Local Read - smaller block size target loads
    // A matrix reads BlockM x BlockK (transposed)
    // B matrix reads BlockK x BlockN
    using LocalReadFragA = wmma::fragment<matrix_b, BlockN, BlockM, BlockK, DataT, LayoutLds>;
    using LocalReadFragB = wmma::fragment<matrix_b, BlockM, BlockN, BlockK, DataT, LayoutLds>;
