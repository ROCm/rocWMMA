...
template <uint32_t BlockM,
          ...
          typename LayoutC,
          typename LayoutD>
__global__ void __launch_bounds__(256, 1) mmaSyncTestLds(uint32_t       m,
                                                          uint32_t       n,
                                                          uint32_t       k,
                                                          InputT const*  a,
                                                          InputT const*  b,
...
struct Kernel
{
    ...
    void MmaSyncTestLdsKernel()
    {
        ...
        hipExtLaunchKernelGGL((mmaSyncTestLds<BlockM,
                                               BlockN,
                                               BlockK,
                                               InputT,
                                               OutputT,
...
template <typename T>
struct MmaSyncTestLdsWrapper;

template<typename BlockM, typename BlockN, typename BlockK, 
         typename InputT, typename OutputT, typename ComputeT>
struct MmaSyncTestLdsWrapper<std::tuple<BlockM, BlockN, BlockK, InputT, OutputT, ComputeT>> : public testing::Test
{
    void SetUp() override 
    {
    // clang-format off
...
TYPED_TEST_SUITE(MmaSyncTestLdsWrapper, Implementations);

TYPED_TEST(MmaSyncTestLdsWrapper, mmasynctestlds)
{
}