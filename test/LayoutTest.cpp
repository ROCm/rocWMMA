                    ...
                    uint32_t  N)
{
    ...
    for(uint32_t i = 0; i < iocount; ++i)
    {
        ...
        {
            auto index = (std::get<MajorIndex>(matrixCoord) * ldm + std::get<MinorIndex>(matrixCoord))
                             + baseOffset + j;
            out[index] = index;
        } 
...
template <uint32_t BlockDim,
          ...
          typename DataT,
          typename DataLayout>
__global__ void colNT(DataT* out,
                      uint32_t  M,
                      uint32_t  N)
{
    enum : uint32_t
    ...
    using IOTraits = amdgcn_io_traits<BlockDim, BlockK, DataT, ElementsPerThread>;
    using LayoutT = Layout::ColNT<BlockDim, BlockK, DataT, DataLayout, ElementsPerThread, MaxVectorWidth>;
    using Mapping = MappingUtil<BlockDim, BlockK, DataT, DataLayout>;
...
template <uint32_t BlockDim,
          ...
          typename DataT,
          typename DataLayout>
__global__ void rowNT(DataT* out,
                      uint32_t  M,
                      uint32_t  N)
{
    enum : uint32_t
    ...
    using IOTraits = amdgcn_io_traits<BlockDim, BlockK, DataT, ElementsPerThread>;
    using LayoutT = Layout::RowNT<BlockDim, BlockK, DataT, DataLayout, ElementsPerThread, MaxVectorWidth>;
    using Mapping = MappingUtil<BlockDim, BlockK, DataT, DataLayout>;
...
struct Kernel
{
           ...
           uint32_t NI)
    {
        ...
        gridDim = dim3(ceilDiv(M,  BlockDim * TBlockX / AMDGCN_WAVE_SIZE), ceilDiv(N, BlockK * TBlockY));
    ...
    void colWrapper()
    {
        ...
        hipLaunchKernelGGL(
            (col<BlockDim, BlockK, DataT, DataLayout>),
            gridDim,
            blockDim,
            0, // sharedMemBytes
            0, // stream
    ...
    void colNTWrapper()
    {
        ...
        hipLaunchKernelGGL(
            (colNT<BlockDim, BlockK, DataT, DataLayout>),
            gridDim,
            blockDim,
            0, // sharedMemBytes
            0, // stream
    ...
    void rowWrapper()
    {
        ...
        hipLaunchKernelGGL(
            (row<BlockDim, BlockK, DataT, DataLayout>),
            gridDim,
            blockDim,
            0, // sharedMemBytes
            0, // stream
    ...
    void rowNTWrapper()
    {
        ...
        hipLaunchKernelGGL(
            (rowNT<BlockDim, BlockK, DataT, DataLayout>),
            gridDim,
            blockDim,
            0, // sharedMemBytes
            0, // stream
...
using Implementations = testing::Types<
    ...
    std::tuple<I<16>, I<16>, float32_t, row_major>, std::tuple<I<16>, I<16>, float32_t, col_major>,
    std::tuple<I<16>, I<16>, float16_t, row_major>, std::tuple<I<16>, I<16>, float16_t, col_major>,
    std::tuple<I<16>, I<16>, hfloat16_t, row_major>, std::tuple<I<16>, I<16>, hfloat16_t, col_major>,
    std::tuple<I<16>, I<16>, int8_t, row_major>, std::tuple<I<16>, I<16>, int8_t, col_major>,
    std::tuple<I<16>, I<16>, int32_t, row_major>, std::tuple<I<16>, I<16>, int32_t, col_major>,
    std::tuple<I<16>, I<16>, uint8_t, row_major>, std::tuple<I<16>, I<16>, uint8_t, col_major>,
    std::tuple<I<16>, I<16>, uint32_t, row_major>, std::tuple<I<16>, I<16>, uint32_t, col_major>,
